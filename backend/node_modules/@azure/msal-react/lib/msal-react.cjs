/*! @azure/msal-react v3.0.9 2025-04-08 */
'use strict';
'use strict';
...
const defaultMsalContext = {
    ...
    accounts: [],
    logger: new msalBrowser.Logger({}),
};
const MsalContext = React__namespace.createContext(defaultMsalContext);
...
function UnauthenticatedTemplate({ username, homeAccountId, localAccountId, children, }) {
    ...
    const isAuthenticated = useIsAuthenticated(accountIdentifier);
    if (!isAuthenticated &&
        context.inProgress !== msalBrowser.InteractionStatus.Startup &&
        context.inProgress !== msalBrowser.InteractionStatus.HandleRedirect) {
        return (React.createElement(React.Fragment, null, getChildrenOrFunction(children, context)));
    }
    return null;
}
...
//# sourceMappingURL=msal-react.cjs.map
